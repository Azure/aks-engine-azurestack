...
install_azure_cli() {
    ...
    echo "deb [arch=amd64 signed-by=/etc/apt/keyrings/microsoft.gpg] https://packages.microsoft.com/repos/azure-cli/ ${CLI_REPO} main" | tee /etc/apt/sources.list.d/azure-cli.list
    apt-get update
    apt-get install -y --allow-downgrades --no-install-recommends azure-cli="${AZCLI_VERSION}-1~${CLI_REPO}"

...
aks-engine-azurestack supports creating a Kubernetes cluster with more than one node pool. These pools can have different configurations, such as VM size or storage profile.
...
When scaling the size of a node pool up or down with aks-engine-azurestack, make sure to specify which node pool:

```sh
$ aks-engine-azurestack scale -g mymultipoolRG --api-model ./multicluster/cluster.json --new-node-count 10 --node-pool gpupool
```

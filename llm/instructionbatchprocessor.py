...
class InstructionBatchProcessor:
    ...
    def _print_copilot_instructions(self) -> None:
        ...
        print()
        print()
        print(f"Please process all instruction files in {output_dir} sequentially and make the required code changes. For each instruction file:")
        print()
        print("1. Read and analyze the instruction file to understand what changes are needed")
        print("2. Identify the target files that need to be modified based on the instruction")
        print("3. Make the necessary code changes using the appropriate edit tools")
        print("4. Validate that the changes were applied correctly")
        print("5. Continue to the next instruction file")
        print()
        print("Process them in alphabetical order starting with:")
        print("- 01-version-map-all-supported-azurestack-windows.md")
        print("- 01-version-map-all-supported-azurestack.md")
        print("- 01-version-map-all-supported.md")
        print("- 02-default-version.md")
        print("- And so on through all files...")
        print()
        print("For each instruction file:")
        print("- Use semantic_search to find relevant code files if needed")
        print("- Use read_file to understand the current state of files before editing")
        print("- Use replace_string_in_file or insert_edit_into_file to make changes")
        print("- Handle any errors gracefully and continue with the next instruction")
        print()
        print("Requirements:")
        print("- Process ALL instruction files without stopping")
        print("- Make actual code changes, don't just analyze")
        print("- Provide a brief status update after processing every 5 instruction files")
        print("- At the end, provide a complete summary of all changes made")
        print("- If any instruction fails, note it but continue with the remaining instructions")
        print()
        print("Context:")
        print(f"- Working directory: {self._code_root_path}")
        print()
        print()
        print("Start processing now and work through all instruction files systematically. Make the actual code changes as specified in each instruction file.")
        print(f"\n{'-' * 80}")

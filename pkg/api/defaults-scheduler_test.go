...
func TestSchedulerFeatureGates132(t *testing.T) {
	...
	s["--feature-gates"] = featuregate132
	cs.setSchedulerConfig()
	actualList := strings.Split(s["--feature-gates"], ",")
	...
	for _, exp := range expectedList {
		...
		if !found {
			t.Fatalf("missing feature-gate %q in actual '--feature-gates' for %s\nfeaturegate131 (expected subset): %q\nactual: %q",
				exp, "1.31.0", featuregate132, s["--feature-gates"])
		}
	}
}

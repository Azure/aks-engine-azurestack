...
func TestSchedulerFeatureGates(t *testing.T) {
	...
	s["--feature-gates"] = featuregate132
	cs.setSchedulerConfig()
	if s["--feature-gates"] != "" {
		t.Fatalf("got unexpected '--feature-gates' for k8s v1.32.0, expected empty string, got: %s", s["--feature-gates"])
	}
	...
	s["--feature-gates"] = featuregate132
	cs.setSchedulerConfig()
	if !strings.Contains(s["--feature-gates"], "CloudDualStackNodeIPs=true") {
		t.Fatalf("expected feature gate CloudDualStackNodeIPs to be present for k8s v1.31.0, got: %s", s["--feature-gates"])
	}
	...
	s["--feature-gates"] = featuregate133
	cs.setSchedulerConfig()
	if s["--feature-gates"] != "" {
		t.Fatalf("got unexpected '--feature-gates' for k8s v1.33.0, expected empty string, got: %s", s["--feature-gates"])
	}
	...
	s["--feature-gates"] = featuregate133
	cs.setSchedulerConfig()
	if !strings.Contains(s["--feature-gates"], "APIListChunking=true") {
		t.Fatalf("expected feature gate APIListChunking to be present for k8s v1.32.0, got: %s", s["--feature-gates"])
	}
	...
	s["--feature-gates"] = featuregate134
	cs.setSchedulerConfig()
	if s["--feature-gates"] != "" {
		t.Fatalf("got unexpected '--feature-gates' for k8s v1.34.0, expected empty string, got: %s", s["--feature-gates"])
	}
	...
	s["--feature-gates"] = featuregate134
	cs.setSchedulerConfig()
	if !strings.Contains(s["--feature-gates"], "DevicePluginCDIDevices=true") {
		t.Fatalf("expected feature gate DevicePluginCDIDevices to be present for k8s v1.33.0, got: %s", s["--feature-gates"])
	}
	...
	s["--feature-gates"] = featuregate135
	cs.setSchedulerConfig()
	if s["--feature-gates"] != "" {
		t.Fatalf("got unexpected '--feature-gates' for k8s v1.35.0, expected empty string, got: %s", s["--feature-gates"])
	}
	...
	s["--feature-gates"] = featuregate135
	cs.setSchedulerConfig()
	if !strings.Contains(s["--feature-gates"], "AllowServiceLBStatusOnNonLB=true") {
		t.Fatalf("expected feature gate AllowServiceLBStatusOnNonLB to be present for k8s v1.34.0, got: %s", s["--feature-gates"])
	}
	if !strings.Contains(s["--feature-gates"], "ComponentSLIs=true") {
		t.Fatalf("expected feature gate ComponentSLIs to be present for k8s v1.34.0, got: %s", s["--feature-gates"])
	}
}

...
func (a *Properties) validateMasterProfile(isUpdate bool) error {
	...
	if to.Bool(m.AuditDEnabled) {
		...
		if a.FeatureFlags.IsEnforceUbuntu2204DisaStigEnabled() && m.Distro != "" && m.IsUbuntu() {
			return errors.New("AuditD should be enabled in all Ubuntu-based pools if feature flag 'EnforceUbuntu2204DisaStig' is set")
		}
	}
...
func (a *Properties) validateAgentPoolProfiles(isUpdate bool) error {
	...
	for i, agentPoolProfile := range a.AgentPoolProfiles {
		...
		if to.Bool(agentPoolProfile.AuditDEnabled) {
			...
			if a.FeatureFlags.IsEnforceUbuntu2204DisaStigEnabled() && agentPoolProfile.IsUbuntu() {
				return errors.New("AuditD should be enabled in all Ubuntu-based pools if feature flag 'EnforceUbuntu2204DisaStig' is set")
			}
		}

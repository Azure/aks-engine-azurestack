...
SUBSYSTEM=="ptp", ATTR{clock_name}=="hyperv", SYMLINK+="ptp_hyperv", RUN+="/bin/sleep 2"
LABEL="ptp_hyperv"
EOF
  udevadm control --reload
  udevadm trigger --subsystem-match=ptp --action=add -v
  [ -e /dev/ptp_hyperv ] && echo "/dev/ptp_hyperv symlink exists" || echo "/dev/ptp_hyperv symlink does not exist"
}
ensureChrony() {

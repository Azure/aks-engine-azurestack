...
// RunUpgrade will upgrade a cluster using the azure cli
func (cli *CLIProvisioner) RunUpgrade() error {
	rgs := make([]string, 0)
	err := cli.upgrade()
	rgs = append(rgs, cli.Config.Name)
	cli.ResourceGroups = rgs
	if err != nil {
		return errors.Errorf("Upgrade failed: %s", err.Error())
	} else {
		if cli.Config.SkipTestsAfterUpgrade {
			return nil
		} else {
			err := cli.waitForNodes()
			return err
...
func (cli *CLIProvisioner) upgrade() error {
	err := cli.Engine.Upgrade(cli.Config.Location, cli.Config.UpgradeVersion)
	if err != nil {
		return errors.Wrap(err, "Error while trying to upgrade aks-engine-azurestack")
	}

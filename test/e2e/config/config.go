...
func (ccc *CustomCloudConfig) SetEnvironment() error {
	...
	// /aks-engine is legacy: cluster.sh (WORK_DIR) now mounts the repo at /aks-engine-azurestack.
	azsSelfSignedCaPath := ""
	for _, p := range []string{"/aks-engine-azurestack/Certificates.pem", "/aks-engine/Certificates.pem"} {
		if _, err = os.Stat(p); err == nil {
			azsSelfSignedCaPath = p
			break
		}
	}
	if azsSelfSignedCaPath != "" {
		...
		// The trailing grep is not fatal on its own (piped through `|| true`); it just leaves an
		// AZURESTACK_CA_CERT_COUNT marker in the log so a run can be confirmed to have picked up the fix.
		...
		echo "AZURESTACK_CA_CERT_COUNT=$(grep -c AzureStackSelfSignedRootCert /etc/ssl/certs/ca-certificates.crt || true)";`, azsSelfSignedCaPath)

		cmd := exec.Command("/bin/bash", "-c", cert_command)

		out, cmdErr := cmd.CombinedOutput()
		log.Printf("azurestack cert install output:\n%s", out)
		if cmdErr != nil {
			return cmdErr
		}
	}
